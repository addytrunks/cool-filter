...
import ProductSkeleton from "@/components/products/ProductSkeletion";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
    ...
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24">
        ...
        <div className="flex items-center">
          <DropdownMenu>
            <DropdownMenuTrigger className="group flex items-center-center text-sem font-medium text-gray-700 hover:text-gray-900">
              Sort
              ...
              {SORT_OPTIONS.map((option) => (
                <button
                  className={cn(
                    ...
                    option.value === filter.sort && "text-gray-900 bg-gray-100"
                  )}
                  key={option.name}
                  onClick={() => {
                    setFilter((prev) => ({
        ...
        <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
          ...
          <ul className='lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8'>
            {products && products.length === 0 ? (
              ...
              products.map((product) => <ProductComponent key={product.id} product={product.metadata!} />)
            ) : (
              new Array(12)
                .fill(null)
